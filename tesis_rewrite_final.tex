...
\author{Henri Paul Camayo Guillermo\\
\small Pontificia Universidad Católica del Perú\\
\small \texttt{henri.camayo@pucp.pe}\\[0.5em]
\small Asesora: Dra. Felícita Doris Miranda Huaynalaya}
...
\begin{abstract}
Esta tesis estudia la representación funcional y la evaluación predictiva de \SVIs{} en mercados de divisas, con énfasis en pares latinoamericanos. El enfoque convierte cada superficie diaria, observada en una malla regular de delta y tenor, en una función bidimensional aproximada mediante una base B-spline tensorial. Sobre esta representación se aplica una corrección métrica mediante la matriz de Gram y una descomposición en componentes principales funcionales (FPCA), lo que permite analizar la estructura latente de las superficies en su geometría natural. La contribución empírica consiste en separar dos preguntas que suelen confundirse: si la FPCA describe de manera parsimoniosa las \SVIs{} y si los modelos dinámicos construidos sobre puntajes FPCA superan a un benchmark de persistencia cruda en pronóstico fuera de muestra.

Con datos diarios de Bloomberg para ocho pares de divisas entre el 4 de febrero de 2025 y el 27 de marzo de 2026, se verifican 299 superficies completas por par, cada una con 65 nodos. Los resultados descriptivos muestran una estructura altamente de baja dimensión: en todos los pares, una o dos componentes explican al menos 95\% de la variación funcional, y dos o tres componentes explican al menos 99\%. En USD/PEN, por ejemplo, las dos primeras componentes explican 98.05\% y las tres primeras 99.37\%. En cambio, el ejercicio predictivo entrega un resultado negativo pero informativo: bajo RMSE sobre la malla observada, la persistencia cruda (PM) obtiene la menor mediana de error en todos los pares y horizontes evaluados (\(h=1,5,10\)). Los modelos PA, VAR(1), ARH en incrementos y KernelARH no logran superar sistemáticamente este benchmark. La evidencia apoya la utilidad descriptiva de la representación funcional y, al mismo tiempo, rechaza la hipótesis de superioridad predictiva de los modelos dinámicos FPCA en este diseño.
\end{abstract}

\textbf{Palabras clave:} volatilidad implícita, superficies funcionales, FPCA, B-splines tensoriales, mercados cambiarios, backtesting, persistencia.
...
\begin{enumerate}
  \item Revisar la literatura sobre FDA, bases B-spline, FPCA y modelos funcionales o multivariados aplicados a superficies de volatilidad.
  ...
  \item Comparar PM, PA, VAR(1), ARH en incrementos y KernelARH mediante RMSE sobre la malla observada, tasas de acierto y pruebas de Diebold-Mariano.
...
\begin{description}
  ...
  \item[H2.] Los modelos dinámicos sobre puntajes FPCA mejoran la precisión fuera de muestra frente a la persistencia cruda. Esta hipótesis es rechazada bajo RMSE mediano sobre la malla observada para los ocho pares y tres horizontes evaluados.
  \item[H3.] La persistencia cruda constituye un benchmark exigente para pronósticos de corto y mediano plazo de \SVIs{}. Esta hipótesis es apoyada fuertemente por la evidencia empírica.
\end{description}

\section{Conceptos básicos sobre opciones y volatilidad implícita}

Una opción \emph{vanilla} otorga el derecho, mas no la obligación, de comprar o vender un activo subyacente a un precio de ejercicio durante un periodo determinado o en una fecha específica. En opciones de divisas, el subyacente es un tipo de cambio, y el valor de la opción depende de variables como el spot, el precio de ejercicio, el plazo, las tasas doméstica y externa, y la volatilidad esperada.

La volatilidad implícita es el valor de volatilidad que, introducido en una fórmula de valorización, iguala el precio teórico de la opción con su precio de mercado. No es una volatilidad histórica observada, sino una variable inferida de precios. En la práctica, para cada fecha se observan volatilidades implícitas asociadas a diferentes deltas y tenores. La colección de estos puntos forma una superficie.

En esta tesis, el eje de delta se define en cinco niveles:
\[
\Delta=\{-0.25,-0.10,0,0.10,0.25\},
\]
correspondientes a 25P, 10P, ATM, 10C y 25C. El eje de tenor contiene trece vencimientos:
\[
T=\{1W,2W,1M,2M,3M,6M,9M,1Y,18M,2Y,3Y,4Y,5Y\}.
\]
Cada superficie diaria se observa entonces en una malla de \(5 \times 13=65\) puntos. Aunque la malla sea discreta, la motivación funcional consiste en aproximar una superficie continua \(Y_t(\delta,\tau)\), donde \(t\) indexa la fecha, \(\delta\) el delta y \(\tau\) el tenor.
...
\subsection{Representación funcional}

Sea \(Y_t(\delta,\tau)\) la \SVI{} observada en la fecha \(t\), con \((\delta,\tau)\in \Delta\times\mathcal{T}\). Aunque los datos se observan en una malla finita, se aproxima cada superficie mediante una base tensorial:
\[
  Y_t(\delta,\tau) \approx \sum_{j=1}^{K_\delta}\sum_{\ell=1}^{K_\tau}
  c_{t,j\ell} B_j^\delta(\delta) B_\ell^\tau(\tau).
\]
En la implementación se usan \(K_\delta=4\), \(K_\tau=8\) y splines cúbicos, por lo que cada superficie se resume en \(K=32\) coeficientes. Si \(x(\delta,\tau)\) denota el vector de bases tensoriales, la superficie se escribe como:
\[
  Y_t(\delta,\tau) \approx x(\delta,\tau)^\top c_t.
\]
Los coeficientes diarios se estiman por mínimos cuadrados ridge:
\[
  \widehat c_t =
  (X^\top X+\lambda I)^{-1}X^\top y_t,
\]
donde \(X\) es la matriz de diseño de dimensión \(65\times 32\), \(y_t\) el vector de volatilidades observadas en la malla y \(\lambda=10^{-6}\) una penalización pequeña para estabilizar la inversión.

\subsection{Métrica funcional y FPCA}

La base tensorial no es ortonormal. Por ello, el producto interno funcional entre dos superficies aproximadas por coeficientes \(c_a\) y \(c_b\) no es \(c_a^\top c_b\), sino:
\[
  \langle f_a,f_b\rangle_{L^2} = c_a^\top G c_b,
\]
donde \(G\) es la matriz de Gram:
\[
  G = \int_{\Delta}\int_{\mathcal{T}} x(\delta,\tau)x(\delta,\tau)^\top d\tau d\delta.
\]
Si \(G=S^\top S\) es la factorización de Cholesky, se transforman los coeficientes como \(u_t=S c_t\). La FPCA se aplica sobre \(u_t\), de modo que las componentes principales son ortogonales en la métrica \(L^2\), no en la métrica euclidiana arbitraria de los coeficientes originales.

Sea \(\bar u\) la media y \(A_{K_0}\) la matriz de las primeras \(K_0\) direcciones principales. Los puntajes son:
\[
  z_t = A_{K_0}^\top (u_t-\bar u).
\]
La reconstrucción funcional desde los puntajes es:
\[
  \widehat u_t = \bar u + A_{K_0}z_t,\qquad
  \widehat c_t = S^{-1}\widehat u_t.
\]

\subsection{Modelos comparados}

El backtest compara cinco pronósticos para \(Y_{t+h}\), con horizontes \(h\in\{1,5,10\}\).

\paragraph{Persistencia en malla observada (PM).}
El benchmark principal copia la última superficie observada:
\[
  \widehat y_{t+h}^{PM}=y_t.
\]
Este modelo no suaviza ni reconstruye la superficie. Por tanto, no incurre en error de base ni de truncación FPCA.

\paragraph{Persistencia de superficie ajustada (PA).}
La persistencia ajustada copia la última superficie suavizada por la base:
\[
  \widehat y_{t+h}^{PA}=X\widehat c_t.
\]
PA permite aislar el costo de pasar de la malla cruda a la representación funcional suavizada.

\paragraph{VAR(1) en puntajes.}
Se ajusta un modelo autorregresivo vectorial de orden 1 en los puntajes:
\[
  z_{t+1}=a+Bz_t+\varepsilon_{t+1},
\]
con regularización ridge. Para horizontes mayores, se itera el sistema.

\paragraph{ARH en incrementos.}
Se modelan los incrementos \(\Delta z_t=z_t-z_{t-1}\) mediante una dinámica lineal:
\[
  \Delta z_{t+1}=\rho\,\Delta z_t+\eta_{t+1}.
\]
El pronóstico del nivel de puntajes se obtiene acumulando el incremento predicho sobre \(z_t\). Esta especificación preserva la persistencia del nivel y estima dinámica en las variaciones.

\paragraph{KernelARH.}
El modelo KernelARH combina una media condicional no paramétrica en el espacio de puntajes con una corrección ARH(1) en residuos. Su motivación es permitir relaciones no lineales, pero su desempeño puede ser sensible al tamaño muestral, al ancho de banda y a la escala de los puntajes.

\subsection{Diseño de backtest}

La evaluación es fuera de muestra y usa ventanas móviles. Para cada par se selecciona una ventana de entrenamiento \(W\in\{44,66,88,110,132,154,176,198\}\) y un número de componentes \(K_0\in\{2,\ldots,15\}\) mediante un procedimiento de tuning interno que usa únicamente información disponible hasta cada fecha de entrenamiento. La FPCA final del backtest se estima de manera local en cada ventana, evitando filtración temporal desde observaciones futuras.

La métrica principal es el RMSE en la malla observada:
\[
  RMSE_{t,h}^{(m)}=
  \left[
  \frac{1}{65}\sum_{r=1}^{65}
  \left(\widehat y_{t+h,r}^{(m)}-y_{t+h,r}\right)^2
  \right]^{1/2}.
\]
Para cada modelo \(m\), par y horizonte se reportan mediana, percentiles 25 y 75, tasa de acierto frente a PM y pruebas de Diebold-Mariano. El diferencial de pérdida se define como pérdida de PM menos pérdida del modelo alternativo. Por ello, un estadístico DM negativo indica que el modelo alternativo tiene mayor pérdida que PM.

\section{Datos}

La base de datos proviene de superficies diarias publicadas por Bloomberg. Luego del filtrado por superficies completas, cada uno de los ocho pares cuenta con 299 fechas desde el 4 de febrero de 2025 hasta el 27 de marzo de 2026. Los pares son USD/PEN, USD/COP, USD/CLP, USD/BRL, USD/ARS, USD/MXN, EUR/USD y USD/ZAR. Cada fecha contiene 65 nodos formados por cinco deltas y trece tenores.
...
Los pares también difieren en el tipo de dinámica latente. USD/PEN es el caso más bifactorial: PC1 explica 74.1\% y PC2 24.0\%, con desplazamientos netos de -3.79 y 2.31 desvíos estándar, respectivamente. Esto indica una rotación importante entre dos factores funcionales, no solo un cambio de nivel. USD/COP también tiene una PC2 persistente, aunque su peso de varianza es menor. USD/CLP, USD/BRL y USD/ARS son más unifactoriales: PC1 explica entre 98.3\% y 98.9\%, por lo que la dinámica global se concentra en un factor de nivel/pendiente dominante. USD/MXN, EUR/USD y USD/ZAR se ubican en una zona intermedia, con PC1 cercana a 95\% y componentes secundarias persistentes que capturan cambios de forma.

Las eigensuperficies ayudan a interpretar esos puntajes. Un movimiento de un desvío estándar en PC1 tiende a afectar más los tenores cortos que los largos en casi todos los pares; la excepción principal es USD/ARS, donde el primer factor carga relativamente más en el tramo largo. En USD/PEN, las tres primeras componentes tienen razones de nivel bajas (entre 0.34 y 0.42), por lo que no son simples desplazamientos paralelos: contienen curvatura y efectos diferenciales por tenor y delta. En USD/COP, USD/CLP, USD/MXN y USD/ZAR, en cambio, algunas componentes secundarias son mucho más cercanas a desplazamientos de nivel, con razones de nivel alrededor de 0.94--0.97. Esta heterogeneidad explica por qué la FPCA es descriptivamente rica, pero también por qué una dinámica común simple en puntajes no se traduce automáticamente en ganancia predictiva uniforme.
...
Primero, los subespacios FPCA locales son estables en términos de varianza explicada. La FVE mediana rolling se mantiene por encima de 0.9989 en todos los pares y horizontes. Esto indica que el problema no es una incapacidad de la FPCA para representar las ventanas locales.
...
La segunda limitación es la métrica de evaluación. El RMSE sobre malla observada favorece modelos que reproducen directamente los nodos crudos. PM no impone suavidad, no reconstruye y no filtra ruido. En aplicaciones de valorización, gestión de riesgo o construcción de superficies económicamente consistentes, podrían ser relevantes otras métricas que ponderen tenores, deltas, no arbitraje o pérdidas económicas.
...
La evidencia también apoya H3. La persistencia cruda es un benchmark difícil porque las superficies son persistentes y porque PM no incurre en errores de suavizado, truncación o estimación. El éxito descriptivo de FPCA no garantiza menor pérdida predictiva sobre una malla cruda.
